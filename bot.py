    ...
    async def handler(event):
        ...
        if cmd.startswith("!setfreq"):
            ...
            try:
                ...
                if len(parts) < 2:
                    await event.reply("❌ Usage: !join <group_invite_link>")
                    return
                link = parts[1].strip()
                await client(JoinChannelRequest(link))
                await event.reply(f"✅ Joined group: {link}")
            except Exception as e:
                await event.reply(f"❌ Couldn't join group: {e}")
            ...
            await event.reply(
                ...
                "!join <invite_link> – Join a new group\n"
